...
def ppt_to_images(file: str, output_dir: str, warning: bool = False, dpi=72, output_type='png'):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        # 核心修复：只在这里加入 Windows 兼容的 URI 转换
        with tempfile.TemporaryDirectory() as user_install_dir:
            safe_user_dir = user_install_dir.replace('\\', '/').replace(':', '|')
            uri_path = f"file:///{safe_user_dir}"
            ...
            command_list = [
                ...
                "--norestore",
                "--nolockcheck",
                f"-env:UserInstallation={uri_path}",
                "--convert-to",
                "pdf",
                file,
                "--outdir",
        ...
        for f in os.listdir(temp_dir):
            ...
            for i, img in enumerate(images):
                save_name = f"poster.{output_type}"
                save_ext = 'PNG' if output_type == 'png' else 'JPEG'
                img.save(pjoin(output_dir, save_name), save_ext)
            return
