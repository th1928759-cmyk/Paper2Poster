...
if __name__ == '__main__':
    ...
    # 新增：字号控制参数
    parser.add_argument('--bullet_font_size', type=int, default=14, 
                        help='font size for bullet points')
    parser.add_argument('--section_title_font_size', type=int, default=24, 
                        help='font size for section titles')
    ...
    if not institution_logo_path:
        ...
        if raw_result:
            ...
            if first_author_inst:
                ...
                if inst_logo_path:
                    institution_logo_path = inst_logo_path.as_posix()
    ...
    if args.conference_venue and not conference_logo_path:
        ...
        if conf_logo_path:
            conference_logo_path = conf_logo_path.as_posix()
    ...
    output_dir = f'[{args.model_name_t}_{args.model_name_v}]_generated_posters/{os.path.basename(args.poster_path).replace(".pdf", "")}'
    ...
    if institution_logo_path:
        institution_clean_path = institution_logo_path.replace('\\', '/')
        print(f'Institution logo added: {institution_clean_path}')
    if conference_logo_path:
        conference_clean_path = conference_logo_path.replace('\\', '/')
        print(f'Conference logo added: {conference_clean_path}')
